...
export function cities(world: WorldState, rng: Rng): void {
  ...
  for (const n of aliveNations(world)) {
    ...
    for (let i = 0; i < world.tiles.length; i++) {
      if (world.tiles[i].owner === n.id && world.tiles[i].city > 0) cityTiles.push(i);
    }
    // 人口与疆域共同决定可供养的城市数
    ...
    // 兴建新城镇：在最宜居处择址（非随机）
    if (cityTiles.length < maxCities && rng.chance(0.14)) {
      let best = -1, bestScore = -1;
      for (let i = 0; i < world.tiles.length; i++) {
        const t = world.tiles[i];
        if (t.owner !== n.id || t.city > 0 || t.dev < 35) continue;
        if (!TERRAIN[t.terrain].passable) continue;
        const dn = nearestCityDist(i, cityTiles, W);
        if (dn < 4) continue;                          // 与既有城市保持间距
        const score = CITY_TERRAIN[t.terrain]
          + t.dev * 0.35
          + waterBonus(world, i)                       // 临水建城
          + Math.min(dn, 9) * 1.1                       // 适度分散
          + rng.next() * 4;
